...
A digital twin as a corrected dynamical system. A model predicts the state forward; observations correct it through the assimilation step; the corrected state re-initialises the model, and the cycle repeats.
```

The idea is deliberately domain-agnostic, and versions of it are now appearing far from the geosciences. In *structural dynamics*, a twin of a bridge or an airframe assimilates strain and vibration measurements to follow how the structure's natural modes shift as load and damage accumulate, turning a fixed finite-element model into a live estimate of fatigue and remaining life. In *power systems*, the electricity grid is a large nonlinear dynamical system whose synchronous operation can be lost through exactly the kind of bifurcations met in chapters {numref}`chap:cont1d`–{numref}`chp:conttwodim`; a twin fed real-time measurements from across the network estimates its state and warns when it is drifting towards instability or voltage collapse. In *physiological flow*, a patient-specific model of blood moving through the heart or arteries, corrected by imaging and pressure data, is used to estimate stresses and flow patterns that cannot be measured directly and to rehearse the effect of an intervention before it is performed. The engineering details could hardly be more different, yet the pattern is in each case the one this chapter has drawn out – a nonlinear model, held on course by data and made affordable by reduction. That this single template serves the atmosphere, a bridge, a power grid and a beating heart is the clearest sign that what the book has taught is not a set of facts about particular systems but a way of reasoning about nonlinear ones.
...
That is the note on which to end. The reader who began by asking how a system as simple as a one-dimensional map or three coupled equations could defeat prediction has, along the way, acquired something more general than a catalogue of chaotic phenomena. The finite horizon that forces us to forecast with ensembles, the correction of models by data, and the low-dimensional structure hidden inside high-dimensional systems are not facts about the Lorenz equations; they are habits of thought that apply wherever nonlinear systems are modelled. The reader has not merely learned about chaos – they have learned to think about nonlinear systems, and that is a tool for the whole of modern science and engineering.
