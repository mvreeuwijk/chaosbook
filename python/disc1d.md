...
:::{only} html
*[Code behind this figure](https://github.com/mvreeuwijk/chaosbook/blob/main/creators/python/disc1d/fig_series_cobweb_return.py)*
:::
...
One can study the cobweb method in more detail by running the Python program below and changing the value of $r$ and/or looking at other mappings like $f(x)=r\sin(\pi x)$ (`cb.sine`).

````{admonition} Python
:class: python

```{code-block} python
# the cookbook shows the cobweb construction written out in plain code
...
:::{only} html
*[Code behind this figure](https://github.com/mvreeuwijk/chaosbook/blob/main/creators/python/disc1d/fig_fixedpoints.py)*
:::
...
In the following Python program we determine the true period-2 solutions for the logistic map, using sympy for the algebra.
...
sympy returns the general solution to $x=g(x)$, where $g$ is given by {eq}`eq:disc1d:p2_g_logisticmap`, as a list:

$$
\mathtt{fp2} = \left\{ 0\,,{1-\frac{1}{r}}\,, {\frac {r+1-\sqrt {{r}^{2}-2\,r-3}}{2r}}\,, {\frac {r+1+\sqrt {{r}^{2}-2\,r-3}}{2r}}\, \right\}
$$

In this set we also recognize the fixed points solutions $x^* = f(x^*)$ derived earlier in {eq}`eq:disc1d:fixed_point_logist`. To get the 'true' period-2 solutions, we calculate the period-1 solutions separately and exclude them:
...
To determine the stability range of the period-2 solutions of the logistic map we extend the code of the previous example by the following commands
...
Numerical solutions to $x=f^{(4)}(x)$ as a function of $r\in[0.1]$ for the sine map $x_{n+1} = f(x_n) = r \sin(\pi x_{n})$ calculated and plotted as a zero contour
```
...
```{code-block} python
# the cookbook shows this construction written out in plain code
